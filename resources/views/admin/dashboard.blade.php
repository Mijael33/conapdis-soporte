            ...
            <div class="col-md-4">
                ...
                <select name="estado_id" id="estadoSelect" class="form-select rounded-3" onchange="cargarSedes()">
                    ...
                    @foreach($estados as $estado)
                        <option value="{{ $estado->id }}" {{ session('filtro_estado_id') == $estado->id ? 'selected' : '' }}>{{ $estado->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="sede_id" id="sedeSelect" class="form-select rounded-3">
                    ...
                    @foreach($sedes as $sede)
                        <option value="{{ $sede->id }}" {{ session('filtro_sede_id') == $sede->id ? 'selected' : '' }}>{{ $sede->nombre_sede }}</option>
                    @endforeach
                </select>
            </div>
...
<div class="row g-3 mb-4">
    ...
    @foreach($resumenGeneral as $stat)
        ...
        <div class="row g-3">
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card operativo">
                    ...
                    <div class="kpi-card-value">{{ $totalEquipos }}</div>
                    <small class="text-muted">{{ $equiposOperativos }} operativos</small>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card mantenimiento">
                    <div class="kpi-card-title">Equipos en Mantenimiento</div>
                    <div class="kpi-card-value">{{ $equiposMantenimiento }}</div>
                    <small class="text-muted">{{ $equiposInoperativos }} inoperativos</small>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card disponible">
                    <div class="kpi-card-title">Componentes Totales</div>
                    <div class="kpi-card-value">{{ $totalComponentes }}</div>
                    <small class="text-muted">{{ $componentesDisponibles }} disponibles</small>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card" style="border-top-color: #d97706;">
                    <div class="kpi-card-title">Órdenes de Servicio</div>
                    <div class="kpi-card-value">{{ $ordenesAbiertas + $ordenesReparadas + $ordenesEsperaRepuesto }}</div>
                    <small class="text-muted">{{ $ordenesAbiertas }} abiertas · {{ $ordenesReparadas }} reparadas</small>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row g-3">
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card disponible">
                    ...
                    <div class="kpi-card-value">{{ $totalBienes }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card operativo">
                    ...
                    <div class="kpi-card-value">{{ $bienesDisponibles }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card">
                    ...
                    <div class="kpi-card-value">{{ $totalBienes - $bienesDisponibles }}</div>
        ...
        <div class="row g-3">
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card" style="border-top-color: #7c3aed;">
                    ...
                    <div class="kpi-card-value">{{ $totalVehiculos }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card operativo">
                    ...
                    <div class="kpi-card-value">{{ $vehiculosDisponibles }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card">
                    <div class="kpi-card-title">Asignados</div>
                    <div class="kpi-card-value">{{ $totalVehiculos - $vehiculosDisponibles }}</div>
        ...
        <div class="row g-3">
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card" style="border-top-color: #0891b2;">
                    <div class="kpi-card-title">Equipos Totales</div>
                    <div class="kpi-card-value">{{ $totalSonido }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card operativo">
                    <div class="kpi-card-title">Disponibles</div>
                    <div class="kpi-card-value">{{ $sonidoDisponible }}</div>
                    <small class="text-muted">listos para asignar</small>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card">
                    <div class="kpi-card-title">Asignados</div>
                    <div class="kpi-card-value">{{ $totalSonido - $sonidoDisponible }}</div>
                    <small class="text-muted">en uso actualmente</small>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
        ...
        <div class="row g-3">
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card" style="border-top-color: #e11d48;">
                    ...
                    <div class="kpi-card-value">{{ $totalEstados }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card" style="border-top-color: #ca8a04;">
                    ...
                    <div class="kpi-card-value">{{ $totalSedes }}</div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="kpi-card" style="border-top-color: #4f46e5;">
                    ...
                    <div class="kpi-card-value">{{ $totalUsuarios }}</div>
...
{{-- ============================================================
     ...
     ============================================================ --}}
@if(session('filtro_sede_id') || session('filtro_estado_id'))
<div class="alert alert-info rounded-4 border-0 shadow-sm">
    <i class="bi bi-info-circle"></i>
    Mostrando datos filtrados por:
    <strong>{{ session('filtro_estado_id') ? \App\Models\Estado::find(session('filtro_estado_id'))->nombre : 'Todos los estados' }}</strong>
    @if(session('filtro_sede_id'))
        - <strong>{{ \App\Models\Sede::find(session('filtro_sede_id'))->nombre_sede }}</strong>
    @endif
</div>
@endif
...
function cargarSedes() {
    ...
    fetch(`{{ url('panel') }}/api/sedes/${estadoId}`)
        .then(r => r.json())
        .then(sedes => {
            sedeSelect.innerHTML = '<option value="">Todas las sedes</option>';
...
<script>
const statsData = [
    { label: 'Equipos', value: {{ $totalEquipos }}, color: '#2563eb' },
    { label: 'Componentes', value: {{ $totalComponentes }}, color: '#059669' },
