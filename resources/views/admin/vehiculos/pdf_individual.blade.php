...
<head>
    <meta charset="UTF-8">
    <title>Ficha del Vehículo - {{ $vehiculo->placa }}</title>
    <style>
...
<body>
    ...
    <table class="datos-table">
        <tr>
            ...
            <td><strong>{{ $vehiculo->codigo_inventario }}</strong></td>
            <td class="label">Placa</td>
            <td><strong>{{ $vehiculo->placa }}</strong></td>
        </tr>
        <tr>
            <td class="label">Categoría</td>
            <td>{{ $vehiculo->categoria->nombre ?? 'N/A' }}</td>
    ...
    <table class="datos-table">
        <tr>
            ...
            <td>{{ $vehiculo->sede->estado->nombre ?? 'N/A' }}</td>
            <td class="label">Sede</td>
            <td>{{ $vehiculo->sede->nombre_sede ?? 'N/A' }}</td>
        </tr>
        <tr>
            <td class="label">Estatus</td>
            <td colspan="3">
                @if($vehiculo->estatus == 'Disponible')<span class="badge-disponible">✓ Disponible</span>
                @elseif($vehiculo->estatus == 'Asignado')<span class="badge-instalado">⚙ Asignado</span>
                @elseif($vehiculo->estatus == 'En Mantenimiento')<span class="badge-revision">⚠ En Mantenimiento</span>
                @else<span class="badge-inoperativo">✗ {{ $vehiculo->estatus }}</span>@endif
            </td>
        </tr>
    </table>

    @if($vehiculo->usuario_asignado_nombre)
    <div class="section-title">Usuario Asignado</div>
    <table class="datos-table">
        <tr>
            <td class="label">Nombre</td>
            <td>{{ $vehiculo->usuario_asignado_nombre }}</td>
            <td class="label">Cédula</td>
            <td>{{ $vehiculo->usuario_asignado_cedula ?? 'N/A' }}</td>
        </tr>
        <tr>
            <td class="label">Cargo</td>
            <td colspan="3">{{ $vehiculo->usuario_asignado_cargo ?? 'N/A' }}</td>
        </tr>
    </table>
    @endif
    ...
    <table class="datos-table">
        <tr><td colspan="4">{{ $vehiculo->observaciones }}</td></tr>
    </table>
    @endif
    ...
    @include('pdf.partials.firmas', [
        ...
        'firma1_cedula' => '',
        'firma2_nombre' => $vehiculo->usuario_asignado_nombre ?? '',
        'firma2_cargo' => 'Funcionario Receptor',
        'firma2_cedula' => $vehiculo->usuario_asignado_cedula ?? '',
    ])
